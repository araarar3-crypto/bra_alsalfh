...
function selectRandomWord(category) {
  ...
  const normalWord = categoryWords[randomIndex].word;
  
  // اختيار كلمة مختلفة تماماً للمندس
  let spyWordIndex;
  do {
    spyWordIndex = Math.floor(Math.random() * categoryWords.length);
  } while (spyWordIndex === randomIndex && categoryWords.length > 1);
  
  const spyWord = categoryWords[spyWordIndex].word;
...
function handleChatMessage(ws, userId, data) {
  ...
  if (!player || !player.roomId) {
    return sendToPlayer(userId, 'error', { message: 'أنت لست في غرفة.' }, ws);
  }
  ...
  if (!message || message.trim().length === 0) {
    return sendToPlayer(userId, 'error', { message: 'الرسالة فارغة.' }, ws);
  }
