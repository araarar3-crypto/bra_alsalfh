...
// المؤقتات
// ============================================
const QUESTION_TIMER_DURATION = 120000; // 2 minutes
const VOTING_TIMER_DURATION = 120000; // 2 minutes
const CHALLENGE_TIMER_DURATION = 120000; // 2 minutes
  ...
  if (room) {
    return room.players.map(userId => {
      ...
      return {
        ...
        isConnected: player.ws && player.ws.readyState === player.ws.OPEN, // ✅ إضافة حالة الاتصال مع التحقق من وجود ws
...
function selectRandomWord(category) {
  ...
  const normalWord = normalWordObject ? normalWordObject.word : 'كلمة عادية';
  ...
  if (otherWords.length > 0) {
    // الأولوية القصوى: اختر كلمة من مجموعة "برا السالفة تماماً" (OtherWords)
    const spyWordIndex = Math.floor(Math.random() * otherWords.length);
    spyWord = otherWords[spyWordIndex] ? otherWords[spyWordIndex].word : 'كلمة مختلفة';
  } else if (similarWords.length > 0) {
    // الأولوية الثانية: اختر كلمة من مجموعة "مشابهة في الوصف" (SimilarWords)
    const spyWordIndex = Math.floor(Math.random() * similarWords.length);
    spyWord = similarWords[spyWordIndex] ? similarWords[spyWordIndex].word : 'كلمة مختلفة';
  } else {
    // كحل أخير: اختر أي كلمة أخرى عشوائياً
    let spyWordIndex;
    do {
 ...
 */
	function generateChallengeWords(category, correctWord) {
	  ...
	  // إذا لم نصل إلى 7 كلمات، نكرر الكلمات الموجودة أو نضيف كلمات وهمية
	  while (incorrectWords.length < 7) {
	    if (incorrectWords.length === 0) {
	      // إذا لم نجد أي كلمة خاطئة، نضيف كلمات وهمية
	      incorrectWords.push(correctWord + " (وهمية 1)");
	      incorrectWords.push(correctWord + " (وهمية 2)");
	    } else {
	      // نكرر الكلمات الموجودة
	      incorrectWords.push(incorrectWords[0]);
	    }
	  }
...
function startNewRound(room) {
  ...
  room.players.forEach(pid => {
    ...
    sendToPlayer(pid, 'gameStarted', {
      ...
      timerDuration: QUESTION_TIMER_DURATION / 1000
    });
  });
...
function startQuestionTimer(room) {
  ...
  room.questionTimer = setTimeout(() => {
    ...
    if (room.playersAsked.size >= room.players.length) {
      ...
      room.playersAsked = new Set();
      broadcastToRoom(room.id, 'votingPhase', { 
    players: getPlayersInRoom(room.id),
    timerDuration: VOTING_TIMER_DURATION / 1000 // إرسال المدة بالثواني
  });
...
function handleFinishQuestion(ws, userId, data) {
  ...
  // تسجيل أن اللاعب قد سأل (سواء كان عاديًا أو مندسًا)
  if (!room.playersAsked) {
    room.playersAsked = new Set();
  }
  ...
  if (room.playersAsked.size >= room.players.length) {
    ...
    broadcastToRoom(room.id, 'nextQuestion', {
      currentPlayer: room.players[room.currentPlayerIndex],
      timerDuration: QUESTION_TIMER_DURATION / 1000 // إرسال المدة بالثواني
    });
  ...
  if (room.votes.length === room.players.length) {
    ...
    if (votedPlayerId === room.spyId) {
      ...
      sendToPlayer(room.spyId, 'spyChallenge', {
        words: challengeWords,
        timerDuration: CHALLENGE_TIMER_DURATION / 1000 // إرسال المدة بالثواني
      });
      ...
      }, CHALLENGE_TIMER_DURATION); // 120 ثانية (دقيقتان)
...
function startVotingTimer(room) {
  const VOTING_TIMEOUT = VOTING_TIMER_DURATION; // 2 دقيقة
...
function handleSpyChallengeAnswer(ws, userId, data) {
  ...
  const { chosenWord } = data; // لا يتم إلغاء مؤقت التحدي بعد رد المندس (حسب طلب المستخدم)
...
function handleKickPlayer(ws, userId, data) {
  const player = players.get(userId);
  if (!player) return;
  ...
  // التأكد من أن اللاعب الذي يطلب الإزالة هو منشئ الغرفة
  if (player.id !== room.creatorId) {
    return sendToPlayer(userId, 'error', { message: 'ليس لديك صلاحية إزالة لاعبين.' });
  }

  const kickedPlayerId = data.targetId;
  const kickedPlayer = players.get(kickedPlayerId);

  if (!kickedPlayer || kickedPlayer.roomId !== room.id) {
    return sendToPlayer(userId, 'error', { message: 'اللاعب المحدد غير موجود في الغرفة.' });
  }

  // التأكد من أن اللاعب المراد إزالته منقطع الاتصال
  if (kickedPlayer.ws.readyState === kickedPlayer.ws.OPEN) {
    return sendToPlayer(userId, 'error', { message: 'لا يمكن إزالة لاعب متصل.' });
  }

  // إلغاء مؤقت الحذف وإزالة اللاعب
  if (kickedPlayer.disconnectTimer) {
    clearTimeout(kickedPlayer.disconnectTimer);
    kickedPlayer.disconnectTimer = null;
  }

  removePlayerFromRoom(room.id, kickedPlayerId);
  console.log(`🗑️ تم إزالة اللاعب المنقطع ${kickedPlayer.name} (${kickedPlayerId}) من الغرفة ${room.roomCode} بواسطة المنشئ.`);
}
...
function handleLeaveRoom(ws, userId, data) {
  ...
  if (room) {
    ...
    if (room.players.length === 0) {
      // ✅ حذف الغرفة إذا لم يتبق فيها أحد، بغض النظر عن الحالة
      cleanupRoomTimers(room);
  ...
  // ✅ حذف اللاعب من خريطة اللاعبين
  players.delete(userId);
  console.log(`👋 غادر اللاعب ${player.name} (${userId}).`);
  
  // ✅ حذف الغرفة إذا لم يتبق فيها أحد وكانت في حالة الانتظار
  if (room && room.players.length === 0 && room.gameState === 'waiting') {
    rooms.delete(room.id);
    console.log(`🗑️ تم حذف الغرفة ${room.roomCode} لعدم وجود لاعبين.`);
  }
}
    ...
    try {
      ...
      switch (event) {
        ...
        case 'reconnect':
          ...
          break;
        case 'kickPlayer':
          handleKickPlayer(ws, userId, data);
          break;
        case 'startGame':
  ...
  ws.on('close', () => {
    ...
    if (player) {
      ...
      // إذا كانت اللعبة جارية، الانتقال لللاعب التالي
	      ...
	      // ✅ حذف الغرفة إذا لم يتبق فيها أحد
	      if (room.players.length === 0) {
	        cleanupRoomTimers(room);
	        rooms.delete(room.id);
	        console.log(`🗑️ تم حذف الغرفة ${room.roomCode} لعدم وجود لاعبين بعد انقطاع الاتصال.`);
	      } else if (room.gameState === 'finished') {
	        // إذا كانت اللعبة قد انتهت بالفعل، مسح المؤقتات المتبقية
	        cleanupRoomTimers(room);
	      }
            }
           }, DISCONNECT_TIMEOUT); // 30 ثانية
