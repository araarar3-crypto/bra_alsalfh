 ...
 * @param {string} oderId - معرف اللاعب
 ...
 * @returns {object} { isBanned, remainingTime, message }
 */
function checkBanStatus(userId) {
  const record = getSpamRecord(userId);
  const now = Date.now();
  
  // فحص إعادة التصفير أولاً
  checkAndResetRecord(record);
  
  if (record.banEndTime > now) {
    const remainingMs = record.banEndTime - now;
    const remainingSeconds = Math.ceil(remainingMs / 1000);
    
    let remainingText;
    if (remainingSeconds < 60) {
      remainingText = `${remainingSeconds} ثانية`;
    } else if (remainingSeconds < 3600) {
      remainingText = `${Math.ceil(remainingSeconds / 60)} دقيقة`;
    } else if (remainingSeconds < 86400) {
      remainingText = `${Math.ceil(remainingSeconds / 3600)} ساعة`;
    } else {
      remainingText = `${Math.ceil(remainingSeconds / 86400)} يوم`;
    }
    
    return {
      isBanned: true,
      remainingTime: remainingMs,
      message: `أنت محظور من الدردشة. المتبقي: ${remainingText}`
    };
  }
  
  return { isBanned: false, remainingTime: 0, message: '' };
}

/**
 * فحص السبام وتطبيق العقوبة إذا لزم الأمر
 * @param {string} userId - معرف اللاعب
 * @returns {object} { isSpam, penaltyApplied, message }
 */
function checkAndHandleSpam(userId) {
  const record = getSpamRecord(userId);
  const now = Date.now();
  
  // تحديد النافذة الزمنية حسب عدد المخالفات
  const timeWindow = record.violations === 0 
    ? SPAM_CONFIG.FIRST_VIOLATION_WINDOW 
    : SPAM_CONFIG.SUBSEQUENT_VIOLATION_WINDOW;
  
  // تنظيف الرسائل القديمة
  record.messageTimes = record.messageTimes.filter(time => now - time < timeWindow);
  ...
  if (record.messageTimes.length >= SPAM_CONFIG.MESSAGE_THRESHOLD) {
    ...
    // تحديد مستوى العقوبة (الحد الأقصى 4)
    const penaltyLevel = Math.min(record.violations, 4);
    const penaltyDuration = SPAM_CONFIG.PENALTIES[penaltyLevel];
    const penaltyMessage = SPAM_CONFIG.PENALTY_MESSAGES[penaltyLevel];
    
    // تطبيق الحظر
    record.banEndTime = now + penaltyDuration;
    
    // تنظيف أوقات الرسائل
    record.messageTimes = [];
    
    console.log(`⚠️ مخالفة سبام #${record.violations} للاعب - حظر حتى ${new Date(record.banEndTime).toLocaleString('ar-SA')}`);
  ...
  return { isSpam: false, penaltyApplied: false, message: '' };
}
...
function handleSkipToVoting(ws, userId, data) {
  ...
  broadcastToRoom(room.id, 'votingPhase', { players: getPlayersInRoom(room.id) });
  console.log(`🗳️ الغرفة ${room.roomCode} انتقلت للتصويت.`);
...
function handleChatMessage(ws, userId, data) {
  ...
  // 📊 Log للتشخيص
  const record = spamRecords.get(userId);
  console.log(`📊 [سبام] اللاعب ${player.name}: عدد الرسائل=${record ? record.messageTimes.length : 0}, المخالفات=${record ? record.violations : 0}`);
  
  if (spamCheck.isSpam && spamCheck.penaltyApplied) {
    console.log(`🚨 [سبام] تم تطبيق عقوبة على ${player.name} - المستوى: ${spamCheck.penaltyLevel}`);
...
function handleLeaveRoom(ws, userId, data) {
  ...
  if (room.gameState === 'waiting') {
    ...
    if (room.players[room.currentPlayerIndex] === userId) {
      ...
      if (connectedPlayers.length === 0) {
        ...
        // البحث عن أول لاعب متصل
        while (!nextPlayer || !nextPlayer.isConnected) {
          nextIndex = (nextIndex + 1) % room.players.length;
          nextPlayer = players.get(room.players[nextIndex]);
  ...
  // 3️⃣ مرحلة التصويت (voting): حذف أصواته وعدم احتساب تصويته
  ...
  // 4️⃣ مرحلة التحدي (challenge): إذا كان المندس هو المنقطع، إلغاء اللعبة
    ...
    if (room.challenge && room.challenge.spyId === userId) {
      if (room.challengeTimer) {
        clearTimeout(room.challengeTimer);
        room.challengeTimer = null;
      }
      
      room.challenge.status = 'disconnected';
      room.gameState = 'finished';
      broadcastToRoom(room.id, 'roundResult', {
        winner: 'normal',
        word: room.currentWord,
        spyWord: room.spyWord,
        spyPlayer: player.name,
        isSpyEliminated: true,
        challenge: {
          status: 'disconnected',
          chosenWord: null,
  ...
  // 5️⃣ مرحلة النتائج (finished): يختفي من القائمة مباشرة
    ...
    console.log(`🗑️ تم حذف اللاعب ${player.name} من القائمة (مرحلة النتائج).`);
  }
  ...
  if (player.isCreator && room.players.length > 0) {
    ...
    if (connectedPlayers.length > 0) {
      const newCreatorId = connectedPlayers[0];
  ...
  // ✅ إذا لم يبق أي لاعب متصل، نحذف الغرفة
  ...
  if (connectedPlayersInRoom.length === 0) {
    ...
    // حذف جميع اللاعبين من الذاكرة
    room.players.forEach(pid => players.delete(pid));
    
    rooms.delete(room.id);
    console.log(`🗑️ تم حذف الغرفة ${room.roomCode} لعدم وجود لاعبين متصلين.`);
  } else {
    rooms.set(room.id, room);
  }
  
  // ✅ إرسال حدث للعميل الذي غادر
...
const gameStats = {
  totalVisits: 0,
  currentPlayers: 0,
  playersSeenAds: new Set()
};

// API: إنشاء إعلان
app.post('/api/ads/create', (req, res) => {
  try {
    const { title, description, link, duration } = req.body;
    const adId = uuidv4();
    const createdAt = new Date();
    const expiresAt = new Date(createdAt.getTime() + parseInt(duration) * 24 * 60 * 60 * 1000);
...
// API: الحصول على الإعلانات النشطة
app.get('/api/ads/active', (req, res) => {
  const now = new Date();
  const activeAds = [];
  
  for (const [id, ad] of ads) {
    if (ad.isActive && ad.expiresAt > now) {
      activeAds.push(ad);
...
// API: تسجيل مشاهدة إعلان
app.post('/api/ads/view', express.json(), (req, res) => {
  try {
    const { adId, playerId } = req.body;
    if (adStats.has(adId)) {
      const stats = adStats.get(adId);
      stats.views++;
      gameStats.playersSeenAds.add(playerId);
    }
...
// API: تسجيل نقرة على إعلان
app.post('/api/ads/click', express.json(), (req, res) => {
  try {
    const { adId } = req.body;
    if (adStats.has(adId)) {
      adStats.get(adId).clicks++;
    }
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
// API: تسجيل إغلاق إعلان
app.post('/api/ads/close', express.json(), (req, res) => {
  try {
    const { adId } = req.body;
    if (adStats.has(adId)) {
      adStats.get(adId).closes++;
    }
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
// API: حذف إعلان
app.delete('/api/ads/:adId', (req, res) => {
  try {
...
// API: الحصول على إحصائيات الإعلانات
app.get('/api/ads/stats', (req, res) => {
  const stats = [];
  for (const [id, ad] of ads) {
    const adStat = adStats.get(id);
    stats.push({
      ...ad,
      ...adStat
    });
...
// API: الحصول على إحصائيات اللعبة
app.get('/api/game/stats', (req, res) => {
  gameStats.totalVisits++;
  gameStats.currentPlayers = players.size;
  
  res.json({
    currentPlayers: gameStats.currentPlayers,
    totalVisits: gameStats.totalVisits,
    playersSeenAds: gameStats.playersSeenAds.size,
    totalAds: ads.size,
    activeAds: Array.from(ads.values()).filter(a => a.isActive && a.expiresAt > new Date()).length
  });
});
