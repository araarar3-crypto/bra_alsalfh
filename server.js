...
function startNewRound(room) {
  ...
  room.players.forEach(pid => {
    ...
    player.stats.gamesPlayed++; // ✅ تحديث عدد الألعاب التي لعبها
    player.stats.timesSpy += player.isSpy ? 1 : 0; // ✅ تحديث عدد مرات المندس
...
function handleVote(ws, userId, data) {
  ...
  if (room.votes.length === room.players.length) {
    ...
    if (votedPlayerId === room.spyId) {
      // 🏆 فوز اللاعبين العاديين بالتصويت
      room.players.filter(pid => pid !== room.spyId).forEach(pid => {
        const p = players.get(pid);
        p.score += 3; // +3 للاعبين العاديين
        p.stats.winsAsNormal++;
        p.stats.spiesCaught++;
      });
      ...
      room.challengeTimer = setTimeout(() => {
        if (room.gameState === 'challenge' && room.challenge && room.challenge.status === null) {
          ...
          // 🏆 فوز اللاعبين العاديين بسبب انتهاء وقت التحدي
          room.players.filter(pid => pid !== room.spyId).forEach(pid => {
            const p = players.get(pid);
            p.score += 1; // +1 للاعبين العاديين
            p.stats.winsAsNormal++;
            p.stats.spiesCaught++;
          });
          ...
          broadcastToRoom(room.id, 'roundResult', {
            ...
            scores: getPlayersInRoom(room.id).map(p => ({ id: p.id, score: p.score }))
          });
...
function startVotingTimer(room) {
  ...
  room.votingTimer = setTimeout(() => {
    ...
    // إذا كان هناك تعادل، يفوز المندس تلقائياً
    if (playersWithMaxVotes.length > 1) {
      // 🏆 المندس يفوز في حالة التعادل
      const spyPlayer = players.get(room.spyId);
      spyPlayer.score += 3; // +3 للمندس
      spyPlayer.stats.winsAsSpy++;
      ...
      console.log(`⏰ انتهى وقت التصويت! تعادل - فاز المندس ${spyPlayer.name} تلقائياً.`);
      rooms.set(room.id, room);
      return;
    }
    ...
    if (votedPlayerId === room.spyId) {
      ...
      console.log(`⏰ انتهى وقت التصويت! فاز المندس ${spyPlayer.name}.`);
    }
...
function handleSpyChallengeAnswer(ws, userId, data) {
  ...
  if (chosenWord === room.challenge.correctWord) {
    ...
    console.log(`🎉 فاز المندس ${spyPlayer.name} بتحدي الكلمات.`);
  } else {
    // 🏆 المندس خسر التحدي، اللاعبون العاديون يفوزون
    const spyPlayer = players.get(room.spyId);
    room.players.filter(pid => pid !== room.spyId).forEach(pid => {
      const p = players.get(pid);
      p.score += 2; // +2 للاعبين العاديين
      p.stats.winsAsNormal++;
      p.stats.spiesCaught++;
    });
    ...
    console.log(`❌ خسر المندس ${spyPlayer.name} تحدي الكلمات.`);
  }
