// ====================================================================
// برا السالفة - خادم اللعبة (إصدار مُحسّن)
// ملاحظات:
// - هذا الملف يعمل مع "type": "module" في package.json
// - يخدم Index.html من نفس المجلد
// - يصلّح مشاكل: creatorId / mapping اللاعب-الغرفة / كانوت جيت /
//   التسريب / بدء اللعبة من غير المضيف / تعادل التصويت /
//   إعادة الاتصال / تنظيف الغرف / إرسال الكلمة بشكل خاص
// - يضيف مرحلة "تخمين الجاسوس النهائي" بعد التصويت
// ====================================================================
...
// كلمة/فئات (مثال مبسّط؛ عدّل بحسب لعبتك الحقيقية)
const WORD_BANK = {
  'السيارات': ['تويوتا', 'هيونداي', 'مرسيدس', 'بي إم', 'جيب', 'كامري', 'سوناتا', 'تيسلا', 'هامِر', 'كيا'],
...
// توليد خيارات تخمين الجاسوس النهائي (٨ كلمات من نفس الفئة تتضمن الكلمة الصحيحة)
...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', (data = {}) => {
    const name = (data.name || 'لاعب').toString().slice(0, 20).trim() || 'لاعب';
    ...
    const room = {
      ...
      // مرحلة تقييم الجاسوس النهائي
      finalGuess: null, // { active: bool, spySocketId, options:[], correctWord }
    ...
    socket.emit('roomCreated', { code: room.code, id: room.id, category: room.category, isHost: true });
  ...
  socket.on('joinRoom', (data = {}) => {
    ...
    // منع انضمام أثناء التخمين النهائي
    ...
    socket.emit('joinedRoom', { code: room.code, id: room.id, category: room.category, isHost: false });
    broadcastRoomState(room);
  });
  ...
  // إنهاء سؤال/دور (تناوب اللاعب المسؤول عن السؤال)
