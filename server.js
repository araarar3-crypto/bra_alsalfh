...
function handleReconnect(ws, userId, data) {
  // نحدد الـ userId المستهدف: إذا أرسله العميل نستخدمه، وإلا نرجع للي مربوط بالاتصال
  const targetUserId = data && data.userId ? data.userId : userId;
  const player = players.get(targetUserId);
  if (!player) {
    return sendToPlayer(
      targetUserId,
      'error',
      { message: 'لا يمكن إعادة الاتصال. لم يتم العثور على بيانات اللاعب.' },
      ws
    );
  }
  ...
  // إذا كان عنده WebSocket قديم مختلف، نحاول نغلقه
  ...
  // نربط اللاعب بالاتصال الجديد
  player.ws = ws;

  // نرسل حالة أساسية للعميل
  ...
  // إذا اللاعب داخل غرفة، نرسل له حالة الغرفة ونبلغ الباقين أنه رجع
  if (player.roomId) {
    const room = rooms.get(player.roomId);
    if (room) {
      // حالة اللعبة الحالية لهذا اللاعب
      sendToPlayer(targetUserId, 'roomState', {
        ...
        isSpy: player.isSpy,
        currentPlayer: room.players[room.currentPlayerIndex],
        creatorId: room.players.find(pid => players.get(pid)?.isCreator) || room.players[0] // البحث عن المنشئ الفعلي
      });

      // تحديث لباقي اللاعبين
