...
function handleCreateRoom(ws, userId, data) {
  ...
  // فحص الأسماء المكررة
  const existingNames = Array.from(players.values()).map(p => p.name.toLowerCase());
  if (existingNames.includes(displayName.trim().toLowerCase())) {
    return sendToPlayer(userId, 'error', { message: 'هذا الاسم مستخدم بالفعل. اختر اسماً آخر.' });
  }
...
function handleVoteNewRound(ws, userId, data) {
  ...
  if (room.newRoundVotes.voters.size === room.newRoundVotes.total) {
    if (room.newRoundVotes.accept === room.newRoundVotes.total) {
      ...
      startNewRound(room);
      broadcastToRoom(room.id, 'newRoundAccepted', {});
