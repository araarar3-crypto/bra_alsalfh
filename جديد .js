...
function startQuestionTimer(room) {
  ...
  room.questionTimer = setTimeout(() => {
    ...
    // ✅ التحقق من انتهاء 15 سؤال أو انتهاء جميع اللاعبين من السؤال
    if (room.questionCount >= room.maxQuestions || room.playersAsked.size >= connectedPlayersCount) {
...
function handleStartGame(ws, userId, data) {
  ...
  room.votes = [];
  room.challenge = null;
  room.questionCount = 0; // ✅ عداد الأسئلة
  room.maxQuestions = 15; // ✅ الحد الأقصى للأسئلة
  room.finishVotingClicks = 0; // ✅ عداد من ضغط زر خلصنا تصويت
...
function handleFinishVoting(ws, userId, data) {
  ...
  if (!room || room.gameState !== 'voting') {
    return sendToPlayer(userId, 'error', { message: 'لا يمكن إنهاء التصويت الآن.' });
  }
  
  // ✅ التحقق من أن اللاعب هو منشئ الغرفة
  if (!player.isCreator) {
    // رسالة للاعب العادي
    return sendToPlayer(userId, 'finishVotingError', { message: 'معليش محد يقدر يضغطه الا منشئ الغرفة 😜' });
  }
  
  // ✅ زيادة عداد من ضغط الزر
  room.finishVotingClicks = (room.finishVotingClicks || 0) + 1;
  
  // ✅ إرسال تحديث لمنشئ الغرفة بعدد من ضغط الزر
  sendToPlayer(userId, 'finishVotingClicks', { clicks: room.finishVotingClicks, totalPlayers: room.players.length });
  
  // ✅ إرسال إشعار لجميع اللاعبين بأن منشئ الغرفة ضغط الزر
  broadcastToRoom(room.id, 'creatorFinishVoting', { clicks: room.finishVotingClicks });
  
  console.log(`👋 منشئ الغرفة ${player.name} ضغط زر خلصنا تصويت (${room.finishVotingClicks})`);
  ...
  if (player.roomId) {
    ...
    if (room) {
      ...
      sendToPlayer(userId, 'roomState', {
        ...
        lastResult: room.lastResult || null // ✅ نتائج الجولة الأخيرة
      });
    ...
    try {
      ...
      switch (event) {
        ...
        case 'chatMessage':
          ...
          break;
        case 'finishVoting':
          handleFinishVoting(ws, userId, data);
          break;
        case 'ping':
